...
check_zig() {
    if ! command -v zig &> /dev/null; then
        print_error "Zig is not installed. Please install Zig 0.17.0-dev (master) or later."
...
# Compose file lives under release/; run this script from the repo root.
COMPOSE="docker-compose -f release/docker-compose.yml"
...
docker_up() {
    ...
    print_info "Check status with: $COMPOSE ps"
    print_info "View logs with: $COMPOSE logs -f"
}
